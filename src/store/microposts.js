    ...
    actions: {
        async getMicroposts({commit}) {
			try {
                ...
                const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71391/LAB8_10/api/microposts.php`)
        ...
        async addMicropost({commit, rootGetters}, post) {
			try {
				...
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71391/LAB8_10/api/microposts.php?session_id=${rootGetters['user/getUser'].session_id}`, {
					method: 'POST',
        ...
        async updateMicropost({commit, rootGetters}, post) {
			try {
                ...
                const response = await fetch(`http://daw.deei.fct.ualg.pt/~a71391/LAB8_10/api/microposts.php?micropost_id=${post.id}&session_id=${rootGetters['user/getUser'].session_id}`, {
					method: 'PUT',
					body: JSON.stringify(post),
                    headers: { 'Content-type': 'application/json; charset=UTF-8' },
				})
		...
		async deleteMicropost({commit, rootGetters},id) {
			try {
                ...
                await fetch(`http://daw.deei.fct.ualg.pt/~a71391/LAB8_10/api/microposts.php?micropost_id=${id}&session_id=${rootGetters['user/getUser'].session_id}`, {
					method: 'DELETE',                
				})
